...
export async function GET(req: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  // Resolve candidate_id by id or slug (so both eduard-buitrago and eduardo-buitrago work).
  ...
  if (!polById?.id) {
    ...
    if (polBySlug?.id) candidateId = String(polBySlug.id);
      ...
      const doneUrl = `${getSiteUrlString()}/connect/done?ok=0&provider=${encodeURIComponent(provider)}&error=candidate_not_found`;
  ...
  if (provider === "meta") {
    return NextResponse.redirect(metaAuthorizeUrl({ clientId, redirectUri, state }));
  }

  if (provider === "reddit") {
    return NextResponse.redirect(redditAuthorizeUrl({ clientId, redirectUri, state }));
  }

  // X requires PKCE. Store verifier in an httpOnly cookie scoped to callback path.
  const { verifier, challenge } = await (async () => {
    const verifier = randomStateToken(48);
    const challenge = sha256Base64Url(verifier);
    return { verifier, challenge };
  })();

  const res = NextResponse.redirect(xAuthorizeUrl({ clientId, redirectUri, state, codeChallenge: challenge }));
  // Store encrypted verifier; short TTL; callback-only path.
  res.cookies.set({
